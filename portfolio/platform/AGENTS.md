...
7. **Fallback de Crédito (Crítico — anti-loop):** Se o BigPickle estiver **sem crédito** (retorno de erro `TIMEOUT`, `sem crédito`, `quota`, `rate limit`, `excedeu`, `not enough credits` OU qualquer erro de ferramenta `ask_bigpickle`), **NÃO retentar em loop** — implementar diretamente no Gemini local (modelo nativo), mantendo as regras 1-5. Nenhuma tarefa pode ficar parada esperando crédito do BigPickle. Uma única tentativa de `ask_bigpickle`; se falhar, seguir no Gemini local imediatamente.
