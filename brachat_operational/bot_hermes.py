...
async def switch_project(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active_path = "/Users/mac/brachat-main/brachat_builder/active_project.json"
    ...
    if os.path.exists(git_dir):
        ...
        src_hook = "/Users/mac/brachat-main/brachat_builder/hooks/pre-commit"
        try:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Prepara o payload no padrão OpenAI que o Hugging Face e a Groq usam
    openai_messages = [{"role": "system", "content": SYSTEM_INSTRUCTION}]
    for msg in chat_history:
        role = "user" if msg["role"] == "user" else "assistant"
        msg_text = msg["parts"][0]["text"]
        openai_messages.append({"role": role, "content": msg_text})
        
    success = False
    reply_text = ""
    active_provider = ""
    
    # 1. Tenta Hugging Face (Llama 3.3 70B via Router)
    hf_key = os.environ.get("HUGGING_API_KEY")
    if hf_key:
        ...
        try:
            logger.info("Tentando Hugging Face (Llama 3.3 70B)...")
            res = await asyncio.to_thread(requests.post, url, headers=headers, json=payload, timeout=25)
            if res.status_code == 200:
                ...
                logger.warning(f"Falha no Hugging Face: Status {res.status_code}. Detalhes: {res.text}")
        except Exception as e:
            logger.warning(f"Erro ao conectar com Hugging Face: {str(e)}")
            
    # 2. Fallback: Tenta Groq (Llama 3.1 8B via CUSTOM_API_KEY)
    if not success:
        groq_key = os.environ.get("CUSTOM_API_KEY")
        if groq_key:
            logger.info("Tentando fallback no Groq (Llama 3.1)...")
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    logger.error(f"Falha no Groq: Status {res.status_code}. Detalhes: {res.text}")
            except Exception as e:
                logger.error(f"Erro ao conectar com Groq: {str(e)}")
    ...
    if success:
        ...
        await update.message.reply_text("⚠️ Falha de cota e limites em todos os modelos de IA (Hugging Face & Groq) configurados.")
