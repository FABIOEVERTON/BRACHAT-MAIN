...
Fábio (Telegram / CLI)
   ...
   ├── 10 Agentes de Estudo (inglês, python, filosofia, pmp, ml, etc)
   ├── 2 Agentes Produtores (job hunter, freelancer)
   ├── 2 Builders (arquiteto + programador)
   └── 24/7 Daemons (Telegram bridges, ClickUp sync)
```
...
**`agents/studies_agents/`** — 12 Agentes de Estudo
...
- `scripts/clickup_daemon.py` — sincronização ClickUp via Composio
...
4. Lê `schedule_progress.json` → descobre o dia atual + `OFICIAL_SCHEDULE.md` → descobre o tópico do dia
...
ClickUp, Cloudflare, Figma, GitHub, Gmail, Google Calendar, Google Drive, Google Meet, Hugging Face, LinkedIn, Mem0, Perplexity AI, Telegram, Trello
