...
- **ID**: `BR-ARCHIT-004`
- **Rule**: Every log, daily plan, commit, or cache/state update generated by me MUST start with `[BR-ARCHIT-004]`.
...
## KNOWLEDGE SOURCE
- Read `builder_agents/architect/cache.json` — previous plan, pending items
- Read `agents/orchestrator_agent/schedule_progress.json` — current day
- Read `writings_studies/` — project context and references
- Consult `agents/studies_agents/*/state.json` — agent availability and progress
