...
1. **Comando Timeout:** Toda execução física de scripts no macOS deve rodar com timeout máximo de **30 segundos**.
...
3. **Inference Routing:** O runtime do Ezra prioriza conexões rápidas por sockets ou chamadas HTTP para o roteador Llama 3.3 no Hugging Face (tempo médio de resposta <1.5s).
...
2. **Lock de Workspace:**
    * O destravamento dinâmico para gravação física do `Coder` (`chmod 644`) expira imediatamente após o encerramento da Fase 5 (Development). O sistema executa `chmod 444` recursivo de forma automática.
