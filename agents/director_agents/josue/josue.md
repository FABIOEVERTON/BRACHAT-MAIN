...
- **ID**: `BR-JOSUE-013`
- **Rule**: Every log, operational status, commit, or state.json update generated by me MUST start with `[BR-JOSUE-013]`.
...
## KNOWLEDGE SOURCE
- Read `director_agents/josue/state.json` — pending operational demands
- Read `agents/orchestrator_agent/schedule_progress.json` — current day and schedule
- Read `writings_studies/` — operational context and reference materials
- Escalate to CEO via orchestrator when resources insufficient
